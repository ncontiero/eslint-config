...
import { type OptionsConfig, ncontiero } from "../src";
...
describe("jsx-a11y rules", () => {
  ...
  it("should catch invalid anchor href", async () => {
    ...
    const errors = results[0].messages.filter((msg) =>
      msg.ruleId?.startsWith("jsx-a11y/"),
    );
  ...
  it("should allow valid accessible JSX", async () => {
    ...
    const results = await eslint.lintText(code, { filePath: "test.jsx" });
    const a11yErrors = results[0].messages.filter((msg) =>
      msg.ruleId?.startsWith("jsx-a11y/"),
    );
  ...
  it("should respect a11y override in ESLint config", async () => {
    ...
    const results = await eslint.lintText(code, { filePath: "test.jsx" });
    const anchorErrors = results[0].messages.filter(
      (msg) => msg.ruleId === "jsx-a11y/anchor-is-valid",
    );
